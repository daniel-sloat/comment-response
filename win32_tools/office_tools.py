#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import logging
from pathlib import Path
import win32com.client as win32


def dispatch_office_app(app: str) -> win32.gencache.EnsureDispatch:
    """Clears win32 gen_py cache in attempt to dispatch office app.
    Example input: "Word", "Excel", etc."""
    # https://gist.github.com/rdapaz/63590adb94a46039ca4a10994dff9dbe
    logging.info(f"Opening Microsoft {app} in background...")
    try:
        office_app = win32.gencache.EnsureDispatch(app + ".Application")
        logging.info(f"Opened Microsoft {app}.")
    except AttributeError:
        # Corner case dependencies.
        import os
        import re
        import sys
        import shutil

        # Remove cache and try again.
        MODULE_LIST = [m.__name__ for m in sys.modules.values()]
        for module in MODULE_LIST:
            if re.match(r"win32com\.gen_py\..+", module):
                del sys.modules[module]
        shutil.rmtree(os.path.join(os.environ.get("LOCALAPPDATA"), "Temp", "gen_py"))
        logging.info("Opened Microsoft {app} (cache cleared).")
        office_app = win32.gencache.EnsureDispatch(app + ".Application")
    return office_app


def mark_index_entries(
    filename: str = "output\CommentResponse.docx",
    automark: str = "output\AutoMark.docx",
    add_index: bool = True,
) -> None:
    word = dispatch_office_app("Word")
    logging.info(f"Opening {filename}...")
    doc = word.Documents.Open(str(Path(filename).absolute()), Visible=False)

    def add_index_entries(doc, automark):
        index = doc.Indexes
        index.AutoMarkEntries(str(Path(automark).absolute()))
        logging.info("Index entries marked.")
        return None

    def append_index(doc):
        docrng = doc.Content  # Select all document range
        docrng.Collapse(0)  # Go to end of range
        docrng.InsertBreak(7)  # Insert page break
        docrng.Collapse(0)
        docrng.Style = -2
        docrng.InsertAfter("Commenter Index\r")
        docrng.Collapse(0)
        docrng.Style = -1

        index = doc.Indexes
        index.Add(Range=docrng, NumberOfColumns=2)
        index.Format = 4
        logging.info("Index appended to end of document.")
        return None

    add_index_entries(doc, automark)
    if add_index:
        append_index(doc)
    doc.Save()
    doc = None
    word.Application.Quit()
    return None
